	...
	return (
		...
		<div>
			...
			<div className={styles.heading}>
				...
				{/* Column */}
				<button className={styles.heading_medium} onClick={() => changeSortOrder(Utils.columns.EMAIL)}>
					<div> {Utils.columns.EMAIL} </div>
					{sortOrder.column === Utils.columns.EMAIL && <SortArrow direction={sortOrder.direction} />}
				</button>
				{/* Column */}
			...
			{clienti.map((cliente) => (
				<Link to={`/clienti/${cliente.id}`} key={cliente.id}>
					...
					<div className={styles.row}>
						<div className={styles.thumbnail}> <img src={Utils.getImageByTipo(cliente.tipo)} alt="img" /> </div>
